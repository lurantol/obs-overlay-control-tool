...
Приложение работает **локально** и управляет OBS через
текстовые файлы (основной и самый надёжный сценарий).
...
Дополнительно (опционально) поддерживается **OBS WebSocket** - для статуса подключения,
проверки Studio Mode и получения скриншотов Program/Preview для удобства оператора.
На сам вывод текста в эфир WebSocket не обязателен.

---

## Логика управления (Preview/Program)

В системе есть 2 разных смысла:

- **Preview (в UI)** - то, что оператор выбирает в `Choose Dancers` (draft).
- **Program (в эфире)** - то, что уже применено кнопками `Apply` (live).

Правила:

- При смене номинации (division) название **сразу применяется в эфир** и пара очищается.
- `Apply Leader` применяет в эфир **только лидера** из выбранного в `Choose Dancers`.
- `Apply Follower` применяет в эфир **только фолловера**.
- `Clear pair` очищает **только пару** (leader/follower), название номинации остаётся.
- `Reset all` очищает и название, и пару (используется редко, отдельная кнопка).

### Scene hotkeys (Studio Mode)

Когда **Studio Mode включён**, под превью появляется панель быстрых переключений сцен.

- Кнопки **1..5** переключают сцены в OBS.
- Горячие клавиши **1..5** делают то же самое.
- Кнопка **Transition** (и горячая клавиша **T**) выполняет переход из Preview в Program (как в OBS Studio Mode).
- В Studio Mode переключается **Preview scene** (как в OBS).
- Если Studio Mode выключен, панель скрыта.

---

## OBS WebSocket (опционально)

Если включить OBS WebSocket (обычно порт `4455`), приложение сможет:

- показывать статус подключения;
- читать флаг `Studio Mode`;
- получать скриншоты `Program` и `Preview` (для контроля перед применением).

### Если OBS WebSocket не работает из-за npm (ECONNRESET / internal registry)

Симптомы:
- `npm i obs-websocket-js` падает с `ECONNRESET`;
- в ошибке фигурирует URL вроде `packages.*.internal.../.../ws-...tgz`.

Причина:
- в `package-lock.json` могли остаться зафиксированные (`resolved`) ссылки на внутренний registry.

Фикс (в корне проекта):
```bash
...
После этого повтори `npm i obs-websocket-js` (если пакет не стоит), и перезапусти сервер.
