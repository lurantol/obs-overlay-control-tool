...
// OBS websocket client is loaded lazily via dynamic import.
// Reason: obs-websocket-js v5 is ESM-only and cannot be loaded via require() in CommonJS.
let obsClient = null;
let obsLibLoaded = false;
let obsLibLoadError = '';

async function getObsClient() {
  if (obsClient) return { ok: true, client: obsClient };
  if (obsLibLoadError) return { ok: false, error: obsLibLoadError };
  try {
    ...
    return { ok: true, client: obsClient };
  } catch (e) {
    obsLibLoaded = false;
    obsLibLoadError = String(e?.message || e || 'Failed to load obs-websocket-js');
    return { ok: false, error: obsLibLoadError };
  }
}
...
async function ensureObsConnected() {
  const loaded = await getObsClient();
  if (!loaded.ok) {
    obsLastErrorAt = Date.now();
    obsLastErrorMessage = loaded.error || 'obs-websocket-js is not available';
...
app.post("/api/operator/clear", (req, res) => {
  // Clear ONLY the current pair. Keep title/division as-is.
  setOverlayStateAndApply({ leader: "", follower: "", withoutPair: false });
  res.json({ ok: true, state: overlayState });
});

// Full reset (title + pair). Keep as a separate action to avoid accidental wipes during the stream.
app.post("/api/operator/reset-all", (req, res) => {
...
app.get('/api/operator/obs-status', async (req, res) => {
  // Load obs-websocket-js lazily so the UI can distinguish between
  // missing dependency and just-not-connected-yet.
  const loaded = await getObsClient();
  const obsAvailable = Boolean(loaded.ok);
  if (loaded.ok) obs = loaded.client;
  const now = Date.now();
